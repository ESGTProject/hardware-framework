...
RUN apt-get update && apt-get install -y python python-pip postgresql-9.5 postgresql-client-9.5 postgresql-contrib-9.5 libpq-dev
...
# Copy files
RUN mkdir /app
ADD run.sh ESGT_setup.py ESGT_database_interface.py ESGT_restful_server.py python-dependencies.txt /app/
WORKDIR /app
RUN chmod +x run.sh
